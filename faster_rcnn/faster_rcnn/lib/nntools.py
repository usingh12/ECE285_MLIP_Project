...
class StatsManager(object):
    ...
    def init(self):
        ...
        self.running_loss = [0, 0, 0, 0]
        self.number_update = 0

    def accumulate(self, x=None, y=None, d=None):
        ...
        """
        self.running_loss[0] += y[1]['loss_classifier'].item()
        self.running_loss[1] += y[1]['loss_box_reg'].item()
        self.running_loss[2] += y[2]['loss_objectness'].item()
        self.running_loss[3] += y[2]['loss_rpn_box_reg'].item()
        
        self.number_update += 1

    def summarize(self):
        """Compute statistics based on accumulated ones"""
        return [self.running_loss[0] / self.number_update, self.running_loss[1] / self.number_update, self.running_loss[2] / self.number_update, self.running_loss[3] / self.number_update]
    ...
    def run(self, num_epochs, plot=None):
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for x, d in self.train_loader:
                ...
                loss = y[1]['loss_classifier'] + y[1]['loss_box_reg'] + y[2]['loss_rpn_box_reg'] + y[2]['loss_objectness']
                loss.backward()
                self.optimizer.step()
                with torch.no_grad():
                    self.stats_manager.accumulate(x, y, d)
