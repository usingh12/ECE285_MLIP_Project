...
from faster_rcnn.utils import _utils as det_utils
...
class RoIHeads(torch.nn.Module):
    ...
    def forward(self, features, proposals, image_shapes, targets=None):
        ...
        if self.training:
            proposals_loss, matched_idxs, labels_loss, regression_targets = self.select_training_samples(proposals, targets)
            
            box_features_loss = self.box_roi_pool(features, proposals_loss, image_shapes)
            box_features_loss = self.box_head(box_features_loss)
            class_logits_loss, box_regression_loss = self.box_predictor(box_features_loss)
            
            loss_classifier, loss_box_reg = fastrcnn_loss(class_logits_loss, box_regression_loss, labels_loss, regression_targets)
