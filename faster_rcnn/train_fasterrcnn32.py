...
class PascalVOCDataset(voc.VOCDetection):
    ...
    def transform__video_voc(self, img, target, img_size=(224, 224)):
        """
        """
    ...
    def transform_voc(self, img, target, img_size=(224, 224)):
        """
        """
        ...
        if thresh < 0.3:    
            ...
            temp = copy.deepcopy(target[:, 1])
            target[:, 1] = img_size[0] - target[:, 3]
            target[:, 3] = img_size[1] - temp
            del temp
            
        elif thresh < 0.6 and thresh >= 0.3:
            transform = tv.transforms.Compose([
            ...
            temp = copy.deepcopy(target[:, 2])
            target[:, 2] = img_size[0] - target[:, 4]
            target[:, 4] = img_size[1] - temp
            del temp
    ...
    def __getitem__(self, index, size=(224, 224)):
        """
        Args:
            index (int): Index
        ...
        yscale = img.size[1]/size[1]
        xscale = img.size[0]/size[0]

        targets = self.get_lbl_data(targets, yscale, xscale)

        if self.transforms is not None:
            img, targets = self.transform_voc(img, targets, size)
...
voc_det_val = PascalVOCDataset(root='VOC2012', year='2012', transforms = True, image_set='val', download=True)

voc_det_test = PascalVOCDataset(root='VOC2007', year='2007', transforms=True, image_set='test', download=True)
...
class FasterRCNNTransfer(NNClassifier):
    ...
    def forward(self, x, d=None):        
        ...
        result, loss_roi = self.fasterrcnn.roi_heads(features_backbone, proposals_rpn, [(224, 224)]*16, targets)
...
exp1 = nt.Experiment(net, voc_det_train, voc_det_test, adam, stats_manager,
                     output_dir="checkpoint32_fasterrcnn", perform_validation_during_training=True, batch_size=32)

torch.backends.cudnn.benchmark=True

print('Beginning Experiment Run...')
